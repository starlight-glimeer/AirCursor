...
const COMPOSITION = `# ⚠️⚠️⚠️ 构图（**这一节比配方更重要** —— 它决定"好不好看"）
...
## 元素
多少个（**上千**，照 ⑤）、什么几何体、多大、怎么排（给出坐标公式）

## 环境
底色（hex）、雾（用不用、near/far 或密度）、灯光（几个/类型/颜色/方向/强度）
⚠️ 照配方里的 environment 做，别落回"深蓝黑底 + 一个顶光"那个默认

## 配色
主色和辅色的 HSL 范围，**以及亮度怎么随"到中心的距离"衰减**（照 ④）

## 音频映射
bass / mid / treble 各驱动什么，具体数值范围。
⚠️ 必须说清**每个元素怎么取到自己那一段频谱**（照 ⑦）

## 三组独立动态
照 ⑥ 列出来：哪个是持续的、哪个是事件触发的、哪个是慢的
