...
// 重新武装：要**持续离开**多久，以及离开的门占触发门的比例。
//
// ⚠️ 位置滞回在这里原理上不成立，这一点是量出来的。手**完全不动**时的逐帧距离实测：
//
//   1.33 0.59 0.35 0.27 0.43 0.54 0.23 0.78 0.18 0.18 0.15 0.18 …
//
// 幅度 0.12–1.33，24 帧里穿越 0.56 这个门 **5 次**。而滞回带（哪怕取 40%）只有 0.22 宽，
// 比噪声幅度小得多 ⟹ 每次"从门外抖回门内"都是一次新触发。扫参证实了这一点：0.5 到 0.95
// 每一档都连发（6 个周期触发 8–34 次），**连接近原设计的 0.95 都是 8 次**。
//
// 所以判据必须带时间：连续离开超过 RE_ARM_MS 才算真的离开了。单帧尖峰跨不过它。
// 这是「帧驱动的门槛判定单位要用时间、不用位置或帧数」在这个项目里的第五次。
//
// 0.6：手松到触发门的六成就算离开 —— 双手门 0.56 时是 0.34，而手指微松实测 0.342 刚好
// 够。共用一个门时它跨不过 0.56，那正是「第一次好触发，后面又很难」。
const RE_ARM_RATIO = 0.6;
// 250ms：和保持判定、丢帧宽限用的是同一个值。真松手很容易超过它，噪声尖峰不会。
//
// ⚠️ **这两个常数都没有标定成功，值是沿用别处的经验值。**
//
// 我做了六次夹具尝试，扫参的结果一直不干净：RE_ARM_MS 从 150 到 600、RE_ARM_RATIO 从
// 0.5 到 0.95，**没有任何一档让单手和双手同时给出"6 个周期触发 6 次"**（结果在 1 到 34
// 次之间跳）。原因是我的夹具"松开段"只有 12 帧 ≈ 516ms，而 RE_ARM_MS=400 就吃掉大半段
// ⟹ 参数和夹具的时间尺度纠缠在一起，扫出来的数字反映的是夹具而不是判据。
//
// 已经确定的是**机制**：位置滞回在这里原理上不成立（手不动时距离在 0.12–1.33 抖，比任何
// 滞回带都宽），所以必须带时间维度。机制成立，参数待真机。
const RE_ARM_MS = 250;
...
class GestureInput {
  ...
  // 保持一个手型会每个冷却期触发一次）。
  staticState(action) {
    // awaySince：从哪一刻开始离开姿势的。0 = 现在没离开。见 RE_ARM_MS。
    if (!this.statics.has(action)) this.statics.set(action, { armed: true, awaySince: 0 });
    ...
    for (const [action, entry] of Object.entries(recorded)) {
      ...
      if (!entry.law) {
        ...
        this.probe.push({
          ...
          threshold: gate,
          armed: state.armed,
          reArm: Number((gate * RE_ARM_RATIO).toFixed(3)),
          awayMs: state.awaySince ? Math.round(now - state.awaySince) : 0,
          why: distance >= gate ? '姿势不够近'
            : state.armed ? '触发'
            // 在滞回带里：够近所以不算离开，但已经触发过 ⟹ 这是"后面很难触发"的那个状态。
            // 报出"离开到多少才重新武装"，否则用户只知道没反应、不知道该松多少。
            : `要先离开到 ${(gate * RE_ARM_RATIO).toFixed(2)} 以上并保持 ${RE_ARM_MS}ms`,
        });
        // ⚠️ 重新武装的门**比触发的门低**，两者不能是同一个数。
        //
        // 共用一个门的后果是"第一次好触发，后面很难"：触发要 `距离 < gate`，而重新武装
        // 要 `距离 >= gate` ⟹ 门放宽一倍之后，"离开姿势"也要离开一倍远才算离开。实测
        // 双手（门 0.56）时手指微松只到 0.342，**仍然算"没离开"**，人得把手完全放开
        // 才能再触发一次 —— 而没人会那么做。
        //
        ...
        //
        // 仿真：摆姿势/微松循环 5 次，单手只触发 2 次、双手 1 次。这就是那个体感。
        //
        // 所以重新武装用 `gate * RE_ARM_RATIO`（0.6）：手离开到"触发门的六成"就算离开了。
        // 这个区间同时也是滞回带 —— 触发和松开之间留一段死区，防止在门附近抖动时连发。
        // 离开了姿势才重新武装 —— 而"离开"要**持续** RE_ARM_MS，见常数注释：
        // 手完全不动时距离也会在 0.12–1.33 之间抖，单帧越线不是离开。
        if (distance >= gate * RE_ARM_RATIO) {
          if (!state.awaySince) state.awaySince = now;
          if (now - state.awaySince >= RE_ARM_MS) state.armed = true;
        } else {
          state.awaySince = 0;
        }
