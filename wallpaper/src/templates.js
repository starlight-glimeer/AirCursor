...
// ⚠️⚠️ **TIERS 不再门控任何东西**（0.9.106）——「进阶模式」那个开关删了，
//   所有动作无条件可用（见下面 actions 那段）。这两条留着只因为面板别处还引用
//   它们的 label/hint 做说明文字，而**它们不再决定谁能用什么**。
//   ⚠️ 别再拿它们当档位用：用户点名「我们就无条件可以使用就好了」。
const TIERS = {
...
const TEMPLATES = {
  depthStage: {
    ...
    // ⚠️⚠️⚠️ **全部动作无条件开放**（0.9.106）。用户 2026-08-02：
    //   「手势这里面有一个进阶模式（手势 + 录制），是一把锁了它以后这些功能才能
    //     使用吗？把它删掉吧，我们就无条件可以使用就好了」
    //
    // **他说得对，而这道门比他以为的更糟**：`basic` 是**空数组** ⟹ 关掉"进阶模式"
    // 的话手势页一个动作都没有（不是"少几个高级功能"，是**整页空白**）。
    // 一个关掉就让整个功能消失的开关，不是"分档"，是个陷阱。
    //
    // ⚠️ 而它存在的理由（分免费/付费档）在这个产品里从来没落地过 ——
    //   `proTier` 默认就是 true，也就是"所有人都在进阶档"。
    //   ⟹ 一个默认打开、关掉就废掉整页的开关 = 纯负担。
    //
    // ⟹ `basic` 收下全部动作，`pro` 留空（保留字段是为了不动 actionsOf 的签名，
    //   而那个函数还有别的调用方）。
    actions: {
      basic: [
        // 壁纸动作
