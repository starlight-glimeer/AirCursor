...
const defaultConfig = {
  ...
  gestureTuning: {
    ...
    // MediaPipe 的手部模型:0=lite(2.1MB) / 1=full(5.5MB)。
    //
    // 默认 1。原来写死 0,理由是"只要掌心和捏合判定"—— 而现在做的是逐点姿势匹配,
    // 每个关键点的精度都进判据。真机实测检出率很差:手在画面里时骨架平均只能连续显示
    // 3 帧(105ms)就断一次。
    //
    // 真机上如果帧率掉了就调回 0(3.x 报告 complexity 0 推理 12ms,full 约 ~24ms,
    // 而帧间隔是 33ms)。
    modelComplexity: 1,
    // 进入跟踪的门槛。
    minDetectionConfidence: 0.5,
    // **维持**跟踪的门槛,比检出低 —— 丢一次跟踪的代价是骨架闪烁 + 预热 3 帧不可信
    // + 序列可能作废,而多认几帧低置信度的手代价很小(判定那边有自己的门)。
