...
//
// ⚠️⚠️ **两个编译坑，第一版全踩了**（用户 2026-08-01 真机报的）：
//
//   ① **中文引号 " " 在 Swift 字符串里必须转义或换掉。**
//      我写了 `"…不会看到"正在共享屏幕"。"` —— 那个 `"` 是 U+201C/201D，
//      Swift 把它当**字符串结束**，于是 `正在共享屏幕` 变成了标识符
//      ⟹ `error: cannot find '正在共享屏幕' in scope`。
//      ⟹ 这个文件里一律用 「」 包中文引用，不用引号。
//
//   ② **嵌套三元 + 字符串拼接会让 Swift 编译器类型检查超时。**
//      `error: the compiler is unable to type-check this expression in reasonable time`
//      ⟹ 那不是语法错，是 Swift 的类型推导在 `?:` 嵌套 + `+` 重载上爆炸。
//      ⟹ 改成 if/else 逐步赋值，一句一个语句。
//
// ⚠️ 而这两个错**云端都查不出**（跑不了 swiftc）—— 一轮真机往返的成本。
// 教训：Swift 里写长文案就用 if/else 组装，别在三元里拼字符串。
...
if #available(macOS 14.2, *) {
    ...
    if tapErr == noErr && tapID != AudioObjectID(kAudioObjectUnknown) {
        ...
        if destroyErr != noErr {
            tapNote += "（销毁失败 \(destroyErr)，重启一下应用）"
        }
    } else {
        tapNote = "建不起来"
    }
    ...
    // ⚠️ **结论用 if/else 组装，不用嵌套三元** —— 见文件头的坑 ②。
    // Swift 的类型检查在「三元嵌套 + 字符串 + 拼接」上会超时。
    var verdict = ""
    if tapErr == noErr {
        if screenOK {
            verdict = "tap 能建，但**这台机器已经有屏幕录制权限** ⟹ 分不清是否依赖它。"
            verdict += "要定案：到 系统设置 → 隐私与安全性 → 屏幕录制 里**关掉** GestureWall"
            verdict += "（或者把这个探针拷到别处、用一个没授权过的身份跑），然后重跑。"
            verdict += "仍然成功 ⟹ 真的不需要屏幕录制，那是一条出路。"
        } else {
            verdict = "tap 能建，而屏幕录制**没有授权** ⟹ **不需要屏幕录制！**"
            verdict += "那就是出路 —— 换过去之后菜单栏不会再显示「正在共享屏幕」。"
        }
    } else {
        verdict = "tap 建不起来（错误 \(tapErr)）"
        if screenOK {
            verdict += "，而屏幕录制**是授权的** ⟹ 这条路本身不通，"
            verdict += "我们现在用 ScreenCaptureKit 是没得选。"
        } else {
            verdict += "，可能是缺屏幕录制权限、也可能这条路本身不通 ⟹ "
            verdict += "先给屏幕录制授权再跑一次：能成功 = 它依赖那个权限，"
            verdict += "仍失败 = 这条路不通。"
        }
    }
    ...
    var old = "这台机器是 macOS \(osStr)，CoreAudio 进程 tap 要 14.2+ ⟹ 用不了。"
    old += "⟹ 这条路即使在新系统上可行，也必须保留 ScreenCaptureKit 作为旧系统的兜底。"
    emit([
        "type": "tapprobe",
        "os": osStr,
        "available": false,
