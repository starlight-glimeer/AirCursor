...
check('⚠️⚠️ 饱和度落在目标区间（低饱和才是"高级感"）', () => {
  // ⚠️ 参考壁纸实测：S 中位 0.30-0.34，激烈时才到 0.65
  //   ⟹ 而模型默认给高饱和霓虹色，那是"一眼就俗"的主因
  const mid = P.satAt(0.5, 0);
  assert.ok(mid >= 0.25 && mid <= 0.40,
    `安静时中间那圈饱和度 ${mid.toFixed(2)} 不在 0.25-0.40（参考是 0.30-0.34）`);
...
check('色相：安静洋红紫 → 激烈蓝紫（参考壁纸的规律）', () => {
  const calm = P.hueAt(0, 0);
  const loud = P.hueAt(0, 1);
  // ⚠️ 实测：安静时洋红紫 290-330 占 40%，激烈时蓝紫 210-260 占 60%
  assert.ok(calm >= 290 && calm <= 330, `安静时色相 ${calm} 不在 290-330`);
