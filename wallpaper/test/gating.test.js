...
check('⚠️⚠️⚠️ 提示词里内置了参考壁纸的实测指标（"这份提示词就是产品"）', () => {
  ...
  // ⚠️⚠️⚠️ **这条守卫的内容 0.9.156 换了一半。** 用户 2026-08-03：
  //   「目标的颜色是可以修改的，没必要紫色，我只是想要实现他那种效果」
  //
  // ⚠️ 我原来把 `#070815` 和 `290-330`（具体色相）当硬指标守 ——
  //   而那是**把"风格"和"审美原理"搞混了**：
  //     那张壁纸恰好用紫色，但紫色不是"那种效果"的来源。
  //   ⟹ 判据：**从参考物量出来的东西要分两类** ——
  //     「关系」（饱和度往外升 / 亮度往外降 / 色相往外偏冷）是效果的来源，**要守**；
  //     「取值」（具体是紫还是青）只是那一张的选择，**不该守**。
  //
  // ⟹ 现在守的是那三条**关系**，以及和颜色无关的那些实测数：
  for (const [num, why] of [
    ['0.30', '中心的饱和度（低饱和 = 偏白，那是"高级感"的来源）'],
    ['0.58', '外围的饱和度 —— 和上面那条一起构成"往外升"这个梯度'],
    ['0.86', '中心亮度'],
    ['0.18', '外围亮度（没入底色）'],
    ['-43', '色相往外偏冷的度数'],
    ['(0, 3, 18)', '相机位置（低但不趴地 —— 0.8 那种会把水面压成一条线）'],
  ]) {
    assert.ok(body.includes(num),
      `提示词里没有 ${num}（${why}）⟹ 那是从参考壁纸实测出来的关系，不是我编的审美`);
  }
  // ⚠️⚠️ 而必须明确说"颜色可以换" —— 否则模型会以为紫色是硬要求
  // ⚠️⚠️ 反向验证逮住这条：我用的是 `A|B` 的或 —— 而删掉"颜色本身你自己定"
  //   之后"没必要紫色"还在（它在引用用户原话那一行）⟹ 守卫照样绿。
  //   ⟹ 判据：**用"或"连接的断言，删掉任一处都不会红** —— 而那两处的作用不同：
  //     一处是指令（"你自己定"），一处是理由（引用用户的话）。两个都要在。
  assert.match(body, /颜色本身你自己定/,
    '没有"颜色本身你自己定"这条**指令** ⟹ 模型不知道它可以换');
  assert.match(body, /没必要紫色/,
    '没引用用户那句原话 ⟹ 少了"为什么可以换"的依据');
  assert.match(body, /这三条关系不能换/,
    '没划出"关系不可换"这条界线 ⟹ 那模型可能连梯度一起改掉');
