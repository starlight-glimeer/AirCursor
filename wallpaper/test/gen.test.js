...
// ═══════════════════════════════════════════════════════════════════════════
//  ⚠️⚠️⚠️ 亮度模型：跟着波纹走，不是固定在中心（0.9.153）
// ═══════════════════════════════════════════════════════════════════════════
console.log('\n  亮度模型与波浪结构');

check('⚠️⚠️⚠️ 提示词不再教"由中心向外衰减"（那条是错的）', () => {
  // ⚠️⚠️ 这是我**教错了六处**的一条。把参考壁纸逐帧量化之后：
  //   帧 30/60 峰值在半径 0（中心）；**帧 90 在半径 12、帧 120 在半径 30**
  //   ⟹ 亮度峰值**在往外移动** —— 那是波纹扩散，不是静态的光球。
  //   而"中心最亮 → 外围没入"只在波纹诞生那一瞬间成立。
  // ⟹ 判据：**我描述的是一个静态构图，而目标是一个动态过程。**
  const plan = G.buildPlanPrompt('');
  assert.match(plan, /亮的地方跟着波纹走/, '没有那一节');
  assert.match(plan, /峰值在往外移动/, '没说清"峰值在移动"这个实测事实');
  // ⚠️ 要摆出那张实测表（帧号 → 峰值半径）—— 那是"我为什么改这条"的证据
  // ⚠️⚠️ 反向验证逮住这条：我原来只查 '12' 和 '30' 这两个数字在不在 ——
  //   而它们在提示词别处也出现（相机参数、其他数值）⟹ 删掉表里那一行照样绿。
  //   ⟹ 判据：**守"那张表本身"**（行数 + 关键行的完整形状）。
  const tableAt = plan.indexOf('| 帧 | 峰值在半径');
  assert.ok(tableAt > 0, '没有那张"帧 → 峰值半径"的实测表');
  const table = plan.slice(tableAt, plan.indexOf('⟹ **看第', tableAt));
  const rows = (table.match(/^\| \d+ \|/gm) || []).length;
  assert.ok(rows >= 6, `实测表只有 ${rows} 行 ⟹ 那张表的说服力就是"峰值在移动"这个趋势，行数少了看不出趋势`);
  // ⚠️⚠️ 而**峰值不在中心**的那两行是核心证据（帧 90 在 12、帧 120 在 30）
  assert.match(table, /\| 90 \| \*\*12\*\*/,
    '缺帧 90 那一行 ⟹ 那是"峰值离开中心"的第一个证据');
  assert.match(table, /\| 120 \| \*\*30\*\*/,
    '缺帧 120 那一行 ⟹ 那是"峰值继续外移"的证据');
  // ⚠️⚠️ 而**必须给出正确的亮度公式**（三项相加）
  assert.match(plan, /底噪/, '没说"底噪很暗，亮的靠加上去"');
  assert.match(plan, /波纹贡献/, '没给波纹那一项');
  assert.match(plan, /频谱贡献/, '没给频谱那一项');
  // ⚠️ 而"亮度 = f(到中心距离)"要被明确否掉
  assert.match(plan, /固定的光球|不管音乐怎么放它都在/,
    '没说清"按距离算亮度会得到一个固定光球" ⟹ 模型还会那么写');
});

check('⚠️⚠️⚠️ "上面暗"不等于"上面空"（0.9.157 教错了两次的那条）', () => {
  // ⚠️⚠️⚠️ **这条测试守的决定翻了。**
  //   0.9.150 我写的是"留白从相机压低来 ⟹ 水面只占画面下半，上半是空的"。
  //   ⟹ 而模型照做了，做出了**只占画面高度 10%** 的一条发光带
  //     （它的代码注释：`SPAN_Z = 7; // 纵深较薄 => 只占中间一横带`）。
  //
  // ⚠️ 而实测参考壁纸（逐行统计"有内容"的像素，阈值 25）：
  //     上 1/3 平均亮度 26，**但 68% 的像素有内容**
  //     主体占画面高度 **88%**（有内容的行 0-124 / 128）
  //   ⟹ 上 1/3 **不是空的** —— 那是远处的水面没入雾里。
  //
  // ⟹ 判据：**"某处暗"和"某处空"是两件完全不同的事**，而我把前者写成了后者。
  const plan = G.buildPlanPrompt('');
  assert.match(plan, /不等于"上面空"/, '没有那一节');
  // ① 要摆出那个实测数字（68% 有内容）—— 那是"为什么不是空的"的证据
  assert.match(plan, /68%/, '没摆出"上 1/3 有 68% 像素有内容"这个实测');
  assert.match(plan, /占画面高度 88%/, '没摆出"主体占 88% 画面高度"这个目标');
  // ②⚠️ 而要给出**两次都做错的那个平衡**（太满 和 太空）
  assert.match(plan, /留白/, '没提留白');
  assert.match(plan, /10%/, '没摆出"一条带只占 10%"这个失败样本');
  // ③⚠️⚠️ 修法三条：纵深要大 / 相机别太低 / lookAt 更远
  assert.match(plan, /Z 从 -60 到 \+10/,
    '没给出具体的纵深范围 ⟹ 模型上次用了 7（那就是"一条带"的来源）');
  assert.match(plan, /相机高度有下界/,
    '没说清相机高度有下界 ⟹ 0.8 那种会把水面压成一条线');
  assert.match(plan, /lookAt.*更远|更远.*lookAt/,
    '没说清 lookAt 的 Z 要比相机更远（沿着水面看向远处）');
  // ④ 而"留白是副产物"这个判据要在
  assert.match(plan, /副产物/,
    '没说清"留白是远处没入雾里的副产物，不是少放元素的结果"');

  // ⑤⚠️⚠️ 而 COMPOSITION 里那条"三分带"也要跟着改 ——
  //   反向验证逮到：我把 ② 那节改回旧标题（"主体在中间那一横条"）时**没报红**，
  //   因为我只守了"一、"那一节。而**同一个错理解在两处** ⟹ 两处都要守。
  //   ⟹ 判据：**同一条判据出现在多处时，每一处都要有守卫** ——
  //     改回任一处都会让模型看到矛盾的指令。
  assert.match(plan, /三分带亮度说的是"同一片水面在不同距离上的亮度"/,
    'COMPOSITION 的 ② 那节没改 ⟹ 它还在说"主体在中间那一横条"，'
    + '和"一、"那节矛盾（模型会照更简单的那条做）');
  assert.match(plan, /暗，但有东西/,
    '② 那节没说清"上 1/3 是暗但有东西"');
  assert.match(plan, /不是"只有中间那一条有元素"/,
    '② 那节没否掉旧的理解');
});

check('⚠️⚠️ 而闸门能逮住"一条带"（真实产物 08030534）', () => {
  // ⚠️ 那张的真实读数：三分带 8/43/8、留白 88%
  //   ⚠️ 它**本身是好看的**（一条发光的波带），但它不是"一片水面"
  const band = { black: 906, total: 1024, bright: 51,
    bands: [8, 43, 8], satMedian: 0.45 };
  const ids = G.judgeComposition({ sampledPixels: band }).map((x) => x.id);
  assert.ok(ids.includes('C-主体是一条带'),
    `没逮住"一条带"（报了 ${ids.join(',') || '（无）'}）`
    + ' ⟹ 三分带里有两带 <12 就是"主体挤在一带里"');
  const d = G.judgeComposition({ sampledPixels: band })
    .find((x) => x.id === 'C-主体是一条带').detail;
  // ⚠️ 报错要给出那两个关键数字 + 四条修法
  assert.match(d, /88%/, '没给出"参考壁纸主体占 88%"这个目标');
  assert.match(d, /68%/, '没说清"上 1/3 是暗而不是空"');
  assert.match(d, /纵深加大/, '没给"纵深加大"这条修法');
  assert.match(d, /相机高度别太低/, '没给"相机别太低"这条修法');

  // ⚠️⚠️ 而参考壁纸的三带（25/90/50）**不能被判成一条带**
  const ref = { ...band, bands: [25, 90, 50] };
  assert.ok(!G.judgeComposition({ sampledPixels: ref }).map((x) => x.id)
    .includes('C-主体是一条带'),
    '参考壁纸的三带被判成"一条带"了 ⟹ 阈值定错了（它的上带是 25，不是 <12）');
  // ⚠️ 而"三带全空"该归 R-画面全黑 管，不该在这里报
  const dark = { ...band, bands: [4, 5, 3], bright: 0 };
  assert.ok(!G.judgeComposition({ sampledPixels: dark }).map((x) => x.id)
    .includes('C-主体是一条带'),
    '三带全空时报了"一条带" ⟹ 那是"整体太暗"的事，两个混在一起会误导');
});

check('⚠️⚠️ "柱子模拟波浪"给了三层叠加的代码', () => {
  // 用户 2026-08-03：「我能明显看出来是用了很多的柱子来模拟波浪，还有涟漪、流星」
  const plan = G.buildPlanPrompt('');
  assert.match(plan, /柱子模拟波浪/, '没引用用户那句话');
  // ⚠️ 三层要都在，而且是**加法**
  assert.match(plan, /空间波打底/, '缺"空间波打底"那一层');
  assert.match(plan, /频谱叠加/, '缺"频谱叠加"那一层');
  assert.match(plan, /涟漪隆起/, '缺"涟漪隆起"那一层');
  assert.match(plan, /三层是加法叠起来的/,
    '没说清三层是加法 ⟹ 模型会写成 if/else 二选一（实测栽过）');
  // ⚠️⚠️ 每层缺了的症状要说清 —— 那让模型能自查
  assert.match(plan, /频谱柱状图/, '没说"缺空间波会变成频谱柱状图"');
  assert.match(plan, /匀速晃|帧间变化会掉/, '没说"缺涟漪会变成匀速晃"');
  // ⚠️ 第二个频率那条（避免棋盘感）
  assert.match(plan, /棋盘感/, '没提"单一频率会有规则的棋盘感"');
});

check('⚠️ "波涛汹涌"拆成三样一起放大', () => {
  // ⚠️ 用户原话里这一句是**可执行的**：不是"整体变亮"，是三样一起放大
  const plan = G.buildPlanPrompt('');
  assert.match(plan, /波涛汹涌/, '没引用那句话');
  // ⚠️⚠️ 反向验证逮住这条：'振幅' 这个词在提示词里出现多次
  ...
  for (const [what, why] of [
    ...
    assert.ok(t2.includes(what), `那张表里缺"${what}"这一行 —— ${why}`);
  }
  // ⚠️ 而每行要给出**具体的从→到**（不是"变大一点"）
  assert.match(t2, /±0\.9.*±4|±4/, '振幅那一行没给出具体幅度');
  // ⚠️⚠️ 而"只调亮度/只调缩放"要被否掉
  assert.match(plan, /只调整体亮度或只调缩放做不出/,
    '没否掉"整体缩放当汹涌"那种做法 ⟹ 那是实测出现过的');
});
