...
check('⚠️⚠️ 留白从"相机压低"来，不是从"少放元素"来', () => {
  // ⚠️ 实测那次：留白 **0%**（目标 20-45%）。
  //   而参考壁纸的留白来源是**画面上 40% 有 27-32% 近黑 = 天空**。
  //   ⟹ 那是相机贴近地平线的结果，不是"元素放少一点"。
  const plan = G.buildPlanPrompt('');
  assert.match(plan, /留白从"相机压低"来/, '没有那一节');
  assert.match(plan, /天空/, '没说清上半是天空');
  // ⚠️ 要给具体的相机参数 + lookAt 的方向（看远处而不是看原点）
  assert.match(plan, /lookAt \(0, 0\.5, -6\)|lookAt.*-6/,
    '没给出"看向远处"的 lookAt ⟹ 看原点的话水面还是会占满画面');
  // ⚠️⚠️ 而"铺满 XZ 平面"要被明确否掉（那是实测的死因）
  assert.match(plan, /铺满 XZ 平面/, '没否掉"铺满整个平面"那种做法');
  assert.match(plan, /留白 \*\*0%\*\*|留白.*0%/, '没摆出那次实测的 0%');
});
