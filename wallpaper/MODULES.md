...
⚠️ **顶部那 25px 不是 bug，是取舍**（我在这上面连错三轮，值得记）。

诊断报告（2026-07-30）：`weBounds` 和 `display` 都是 `1470×956`，**一像素不差**，
`pushes: 0` 说明窗口压根没被夹过 —— 而用户仍然看到顶上那条带子。
...
|---|---|---|
| `desktop` | ✅ 有内容 | ❌ 收不到 |
| `bottom-normal` | ⚠️ 那 25px 是系统的 | ✅ 能收 |

⟹ 二者不可兼得，所以做成了面板上的开关，不替用户决定。

⚠️ **教训**：三轮都在修"窗口尺寸不够大"（推三次 → 定时轮询 → 事件驱动），
而尺寸从头到尾都是对的。**尺寸是可测的，而我三次都没去测** ——
第三版加的那个核对一上线就证明前提是错的。
⟹ 报出可核对的数字 > 修得更用力。
