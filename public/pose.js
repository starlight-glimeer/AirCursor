...
// 匹配阈值不按手数放宽 —— 这个函数留着是为了让"不放宽"这个决定有地方写清楚。
//
// ⚠️ 曾经这里是 `hands >= 2 ? threshold * 2 : threshold`，依据是"双手命中率只有 51%"。
// **那个数是假的**：它来自一个把真机逐帧增量累加到静止基准上的夹具，而增量累加会随机
// 游走式堆积 ⟹ 实测抖动被夸大 **5.7 倍**（相邻帧距离中位 0.275，真机 0.048）。
//
// 用真机绝对帧重测（各取手最静止的那个窗口）：
//
//            离自己模板的距离   门 0.28 的命中率
//   单手       中位 0.050          **100%**
//   双手       中位 0.079          **100%**
//
// 双手只比单手噪声大 1.6 倍，而 0.28 对两者都是 100% 命中。放宽一倍纯属多余，而且它
// 直接引出了下一个 bug：那个门同时是"离开姿势"的判据，放宽之后双手触发一次就要把手
// 完全放开才能再触发（用户报的「第一次好触发，后面又很难」）。
//
// 双手真正难触发的原因是**丢跟踪**：真机双手帧只占 69%，而双手连续段的中位长度只有
// 3 帧（129ms）。那条已经单独修了（SequenceMatcher.excuse 把丢帧的时间还给序列）。
//
// 教训：**先验证夹具，再用它标定承重常数。**一个夸大 5.7 倍的噪声模型会让每个阈值都偏大，
// 而偏大的阈值不报错、只是让判别力变差，然后引出下一轮"为什么手势互相串"。
