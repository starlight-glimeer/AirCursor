...
(() => {
  ...
  let yaw = 0.35;
  // ⚠️⚠️ pitch 从 -0.22 改成 **-0.30**（0.9.110）—— 用户 2026-08-02 的截图里
  //   一圈卡片**挤成扁扁一团**（量过：横向只占屏宽 59%、竖向 24%）。
  //   俯角太小 ⟹ 圆环投影成一条很扁的椭圆 ⟹ 看起来是"一堆方块"而不是"环绕"。
  let pitch = -0.38;
  ...
  function rebuild() {
    ...
    for (let r = 0; r < rings; r += 1) {
      ...
      // ⚠️⚠️⚠️ **两轮都搜错了目标**（0.9.112）。
      //
      // 0.9.110 我按"横向铺满 88%"搜参，结果 42 张卡片里 **27 张小于 40px**
      // ⟹ 用户 2026-08-02 的截图里那些远处的卡片糊成一片小灰块，
      //   而只有近处 7 张看得出是封面 ⟹ 观感是"卡片挤在左半边、右边空着"，
      //   **而我量过 x 重心正好是 960（屏幕正中）** —— 偏的不是位置，
      //   是**能看清的那几张恰好偏在一侧**。
      //
      // ⟹ 目标改成：**最小卡片 >= 52px**（那才看得出是张封面）。
      //   重搜（R0/RS/卡片系数/camZ/pitch/每圈数量/圈数 七个量）⟹
      //   **3 圈 × 每圈 8 张 = 24 张，最小 56px、最大 354px，横 77% 竖 55%**。
      //
      // ⚠️ 关键取舍：**卡片从 42 张减到 24 张**。少了反而好看 ——
      //   42 张里一半是看不清的噪点，而"一堆糊掉的小方块"比"少一点但都清楚"糟。
      //   ⟹ 判据：**能看清的元素数量才算数量，画上去的不算。**
      const radius = 1.8 + r * 0.7;
  ...
  function project(x, y, z) {
    ...
    // ⚠️ 5.0（0.9.110 从 4.6 提上来）—— 半径变大之后要把镜头拉远，
    //   否则近处的卡片穿到镜头背后（denom < 0.25 那条会把它们整片剔掉）。
    const camZ = 4.0 * dist;
    const denom = z2 + camZ;
    if (denom < 0.25) return null;          // 背后的，不画
    // ⚠️⚠️⚠️ **scale 必须夹上限**（0.9.110）。我自己的采样脚本逮到的：
    //   遍历全部 yaw/pitch/dist 组合后，最坏情况下卡片会算到 **606px**
    //   （屏高 1080 的一半多）、整圈溢出屏幕 8733px。
    //   那发生在"滚轮拉到最近 + 俯角拉到极限"时 —— 近处那张卡片糊住整个画面。
    // ⚠️ 而这**不是**理论隐患：dist 下限 0.55、pitch 下限 -1.15 都是用户拖得到的。
    //   ⟹ 夹住。代价是极近处失去"冲脸"的透视感，收益是任何视角下都不糊屏。
    const scale = Math.min(3.0 / denom, 4.2);
    const base = Math.min(W, H);
    return {
      // ⚠️ 0.20（从 0.30 降）—— 半径大了，投影系数要跟着降，否则整圈飞出屏幕。
      sx: W / 2 + x1 * scale * base * 0.20,
      sy: H / 2 + y1 * scale * base * 0.20,
      scale,
      depth: denom,
    };
  }
  ...
  function frame(now) {
    ...
    for (const { c, p } of drawn) {
      // ⚠️ 0.05（从 0.085 降）—— 见上面 radius 那段：搜参时把"最大卡片
      //   < 屏高 1/4"当硬约束，这个系数是那个约束的直接来源。
      const size = p.scale * Math.min(W, H) * 0.115;
