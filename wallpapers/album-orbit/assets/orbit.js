...
(() => {
  ...
  function rebuild() {
    ...
    for (let r = 0; r < rings; r += 1) {
      ...
      // ⚠️⚠️ 半径从 0.9/+0.72 改成 **1.8/+1.1**（0.9.110）。
      //   原值算出来横向只占屏宽 59%、竖向 24% ⟹ 挤成一团（用户截图）。
      //   ⚠️ 这组数是**搜出来的**（穷举 R0/RS/YS/SP/PS/camZ/pitch 七个量，
      //     目标：横 78~95%、竖 45~72%、最大卡片 < 屏高 1/4、最小 > 14px）
      //     ⟹ 现在横 88%、竖 54%、卡片 19~149px。
      //   ⚠️ 而"最大卡片 < 屏高 1/4"那条是必须的：近处那张要是占半屏，
      //     它会糊住整个画面（那比挤成一团更难看）。
      const radius = 1.8 + r * 1.1;
  ...
  function frame(now) {
    ...
    for (const { c, p } of drawn) {
      ...
      const size = p.scale * Math.min(W, H) * 0.05;
