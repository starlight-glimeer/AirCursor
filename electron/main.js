...
const defaultSettings = {
  ...
  twoHands: true,
  effects: "balanced",
  // Empty: every gesture is recorded by the user. See defaultGestureMap below for why
  // the built-in shapes were dropped.
  gestureMap: {},
  recordedGestures: {},
...
// from click or wake.
// Nothing is bound by default: every action is recorded by the user.
//
// This replaces a set of built-in hand shapes (pinch / middle-pinch / open palm /
// fist) that was measured to have a low ceiling — four shapes for nine actions, so
// `drag` could not be assigned one at all, and the user's verdict on the ones that
// were assigned was that they work but not well. Hand-written shape tests are a
// weaker signal than a template the user recorded with their own hand.
//
// The consequence is that a fresh install does nothing until gestures are recorded,
// which is deliberate for now: the plan is to ship a few of the user's own recordings
// as factory defaults once the product settles, and those have to come from a real
// hand. Synthesising them here would produce defaults that look present and match
// nothing — the same synthetic-vs-real gap that has cost this project several rounds.
//
// Kept as an empty object rather than removed: clearing a gesture reads this to decide
// what to fall back to, and `?? null` then correctly means "unbound".
